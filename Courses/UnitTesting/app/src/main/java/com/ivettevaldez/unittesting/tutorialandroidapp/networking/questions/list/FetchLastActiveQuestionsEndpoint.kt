...
open class FetchLastActiveQuestionsEndpoint(private val stackOverflowApi: StackOverflowApi?) {
    ...
    open fun fetchLastActiveQuestions(listener: Listener) {
        stackOverflowApi!!.fetchLastActiveQuestions(
            Constants.QUESTIONS_LIST_PAGE_SIZE
        )!!.enqueue(getCallback(listener))
    }

    private fun getCallback(listener: Listener): Callback<QuestionsListResponseSchema?> =
